...
# Land area fraction ----------------------------------------------------------------------------------------------
if (!file.exists(file.path(raw_data_path, "BARRA_R2_cell_mask.Rdata")) | overwrite == T) {
  land_path <- file.path(raw_data_path, "sftlf_AUS-11_ERA5_historical_hres_BOM_BARRA-R2_v1.nc")
  
  nc <- nc_open(land_path)
  lat <- ncvar_get(nc, "lat")
  lat_ind <- which(lat > -45 & lat < -9.6)
  ...
  # Cut out Indonesia before doing the distance calculation to reduce computational load
  ...
  # Create raster of distance from land values
  # dist <- distance(rast) # takes ages, load if you can
  # writeRaster(dist, file.path(raw_data_path, "BARRA_R2_fulldist.tif"))
  
  dist <- raster(file.path(raw_data_path, "BARRA_R2_fulldist.tif"))
  dist[dist > 72*10^3] <- NA
  plot(dist)
  
  # Mask out cells too far from the coast
  ...
  save(lat, file = file.path(raw_data_path, "BARRA_R2_lats.Rdata"))
  save(lon, file = file.path(raw_data_path, "BARRA_R2_lons.Rdata"))
  save(lat_ind, file = file.path(raw_data_path, "BARRA_R2_lat_inds.Rdata"))
  save(lon_ind, file = file.path(raw_data_path, "BARRA_R2_lon_inds.Rdata"))
  save(cell_mask, file = file.path(raw_data_path, "BARRA_R2_cell_mask.Rdata"))
  
  nc_close(nc)
  rm(nc, dist, land, sea, rast)
} else {
  load(file.path(raw_data_path, "BARRA_R2_lats.Rdata")) # lat
  load(file.path(raw_data_path, "BARRA_R2_lons.Rdata")) # lon
  load(file.path(raw_data_path, "BARRA_R2_lat_inds.Rdata")) # lat_ind
  load(file.path(raw_data_path, "BARRA_R2_lon_inds.Rdata")) # lon_ind
  load(file.path(raw_data_path, "BARRA_R2_cell_mask.Rdata")) # cell_mask
}
clean_memory()
...
for (var in vars) {
  ...
  rast_files <- data.frame(
    fnm = list.files(file.path(var_rasters), full.names = T),
    ...
    mutate(
      ...
      leap_day = case_when(
        ...
        leap_year == T &
          yday(time) > yday(as.Date("2024-02-28")) ~ yday(time) - 1,
        T ~ yday(time)
      )
    )
  ...
  for (d in unique(rast_files$leap_day)) {
    ...
    if (!file.exists(fname) | overwrite == T) {
      rf <- rast_files[rast_files$leap_day == d & str_detect(rast_files$date, "2017|2018|2019|2020|2021|2022|2023"), ]
...
writeRaster(x = land_rast, filename = file.path("D:", "FRDC-Seaweed-Raw-Data", "BARRA-R2", "BARRA_R2_land_rast.tif"), overwrite = T)

doy_raster_means <- file.path(raw_data_path, "rsdsdir", "raster_means") %>% 
  list.files(full.names = T)
BARRA_R2_rsdsdir_data <- terra::rast(doy_raster_means)
writeRaster(x = BARRA_R2_rsdsdir_data, filename = file.path(final_data_path, "BARRA_R2_rsdsdir_data.tif"), overwrite = T)
rm(BARRA_R2_rsdsdir_data)

doy_raster_means <- file.path(raw_data_path, "ts", "raster_means") %>% 
  list.files(full.names = T)
BARRA_R2_ts_data <- terra::rast(doy_raster_means)
writeRaster(x = BARRA_R2_ts_data, filename = file.path(final_data_path, "BARRA_R2_ts_data.tif"), overwrite = T)
rm(BARRA_R2_ts_data)
