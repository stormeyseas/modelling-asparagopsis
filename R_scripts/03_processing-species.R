...
temp$a_armata <- sapply(X = temp$temp, FUN = T_lim, spec_params = a_armata)
temp$a_taxiformis <- sapply(X = temp$temp, FUN = T_lim, spec_params = a_taxiformis)
temp <- temp %>%
  pivot_longer(cols = c("a_armata", "a_taxiformis"), names_to = "species", values_to = "Tlim") %>%
  write_parquet(file.path(output_path, "T_response.parquet"))
rm(temp)

## Salinity --------------------------------------------------------------------------------------------------
sali <- data.frame(sali = seq(0, 65, 0.25))
sali$a_armata <- sapply(X = sali$sali, FUN = S_lim, spec_params = a_armata)
sali$a_taxiformis <- sapply(X = sali$sali, FUN = S_lim, spec_params = a_taxiformis)
sali <- sali %>%
  pivot_longer(cols = c("a_armata", "a_taxiformis"), names_to = "species", values_to = "Slim") %>%
...
for (d in 1:length(culture_depths)) {
  light$a_armata <- sapply(X = light$light, FUN = I_lim, kW = 0.1, Nf = 500, site_params = c(d_top = culture_depths[d], kW = 0.2), spec_params = a_armata)
  ls1[[d]] <- light %>%
    pivot_longer(cols = c("a_armata"), names_to = "species", values_to = "Ilim") %>%
...
sens_N_end_taxi <- sens_N_end_taxi %>% 
