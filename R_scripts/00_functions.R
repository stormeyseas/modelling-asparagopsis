...
fill_raster_gaps <- function(raster, cell, output) {
  if (any(is.na(output))) {
    ...
    output <- terra::extract(raster, neighbours) %>% colMeans(na.rm = T)
  }
  if (any(is.na(output))) {
  ...
  if (any(is.na(output))) {
    neighbours <- terra::adjacent(raster, neighbours, "16") %>% as.vector() %>% unique()
...
# raster <- BRAN_salt_raster
# point <- c(cell_coords$lon, cell_coords$lat)

extract_stepwise_buffer <- function(raster, point, distances = c(1000, 5000, 10000, 25000, 50000, 75000)) {
  # Get layer names for output
  layer_names <- names(raster)
  
  # Try exact point first
  point_vect <- terra::vect(matrix(point, ncol = 2), crs = terra::crs(raster))
  exact_values <- terra::extract(raster, point_vect, method = "simple", ID = F) %>% 
    unlist() %>% unname()
  
  if (all(is.na(exact_values))) {
    # Try each distance buffer
    for (dist in distances) {
      buffer_poly <- terra::buffer(point_vect, width = dist)
      buffered_values <- terra::extract(raster, buffer_poly, method = "simple", ID = F)
      
      # Calculate mean for each layer, excluding NAs
      layer_means <- numeric(terra::nlyr(raster))
      names(layer_means) <- layer_names
      
      for (i in seq_along(layer_names)) {
        layer_means[i] <- mean(buffered_values[, i], na.rm = TRUE)
      }
      values <- unname(layer_means)
