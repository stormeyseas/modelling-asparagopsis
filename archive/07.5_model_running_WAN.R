...
tar_option_set(
  ...
  format = "qs", 
  controller = crew_controller_local(workers = parallel::detectCores()-5, seconds_idle = 120),
...
list(
  ...
  tar_target(
    ...
    command = {
      ...
      df$Ni_input <- df$Ni_input + rnorm(730, 0, 0.1)* Ni_data_prioritised[["sd"]]
      df$Am_input <- df$Am_input + rnorm(730, 0, 0.1)* Am_data_prioritised[["sd"]]
  ...
  tar_target(
    ...
    command = {
      if (site_params['hz'] > (site_params['d_top'] + site_params['hc'])) {
        if(any(is.na(cell_input_timeseries$S_input[start_ydays_growth:(start_ydays_growth + 42)]))) {
          salinity <- state_input_timeseries$S_input_mean[start_ydays_growth:(start_ydays_growth + 42)]
        } else {
        ...
        if(any(is.na(cell_input_timeseries$UV_input[start_ydays_growth:(start_ydays_growth + 42)]))) {
          velocity <- state_input_timeseries$UV_input_mean[start_ydays_growth:(start_ydays_growth + 42)]
        } else {
    ...
    pattern = cross(
      map(species_data, init_state_all), 
  ...
  tar_target(
    ...
    command = {
      mat <- grow_macroalgae(
        start =        start_ydays_growth,
        grow_days =    42,
        temperature =  state_input_timeseries$T_input_mean[start_ydays_growth:(start_ydays_growth + 42)] *
          theo_scens$T_mod[theo_scens$name == theo_names],
        salinity =     state_input_timeseries$S_input_mean[start_ydays_growth:(start_ydays_growth + 42)] *
          theo_scens$S_mod[theo_scens$name == theo_names],
        light =        state_input_timeseries$I_input_mean[start_ydays_growth:(start_ydays_growth + 42)],
        kW =           state_input_timeseries$Kd_490_mean[start_ydays_growth:(start_ydays_growth + 42)] *
          theo_scens$kW_mod[theo_scens$name == theo_names],
        velocity =     state_input_timeseries$UV_input_mean[start_ydays_growth:(start_ydays_growth + 42)] *
          theo_scens$UV_mod[theo_scens$name == theo_names],
        nitrate =      state_input_timeseries$Ni_input_mean[start_ydays_growth:(start_ydays_growth + 42)] +
          theo_scens$Ni_add[theo_scens$name == theo_names],
        ammonium =     state_input_timeseries$Am_input_mean[start_ydays_growth:(start_ydays_growth + 42)] +
  ...
  tar_target(
    theo_growth_lims,
    get_limiting(theo_scen_growth, spec_params = unlist(species_data)),
