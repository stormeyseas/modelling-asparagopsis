...
cells_by_station_1 <- cells_by_station %>% filter(data_source == "refstation")
...
# levels(cells_by_station$name)
```

Matching model cells to their two nearest reference stations resulted in 
`r round(100*cells_by_station_1$prop[cells_by_station_1$name == "Darwin"], 1)`% of cells using data from Darwin, 
`r round(100*cells_by_station_1$prop[cells_by_station_1$name == "Kangaroo Island"], 1)`% using data from Kangaroo Island, 
`r round(100*cells_by_station_1$prop[cells_by_station_1$name == "Maria Island"], 1)`% using data from Maria Island, 
`r round(100*cells_by_station_1$prop[cells_by_station_1$name == "North Stradbroke Island"], 1)`% from North Stradbroke Island, 
`r round(100*cells_by_station_1$prop[cells_by_station_1$name == "Port Hacking"], 1)`% using data from Port Hacking, 
`r round(100*cells_by_station_1$prop[cells_by_station_1$name == "Rottnest Island"], 1)`% using data from Rottnest Island, and 
`r round(100*cells_by_station_1$prop[cells_by_station_1$name == "Yongala"], 1)`% using data from Yongala. 
...
armata_lims <- cell_growth_lims_fnms %>% 
  purrr::map(function(fnm) {
    arrow::read_parquet(fnm) %>%
      ...
      mutate(t = case_when(t > 365 ~ t-365, T ~ t), t = as.integer(t)) %>% 
