...
packs <- c('units', 'cowplot', 'formatdown', 'knitr', 'macrogrow', 'conflicted', 'magrittr', 'stringr', 'dplyr', 'tidyr', 'cowplot', 'formatdown', 'knitr', 'macrogrow', 'conflicted', 'units', 'ggplot2', 'here', 'qs') %>% unique() %>% sort()
...
V_am <- K_am <- M_am <- C_am <- V_ni <- K_ni <- M_ni <- C_ni <- V_ot <- K_ot <- M_ot <- C_ot <- Q_min <- Q_max <- K_c <- mu <- N_min <- N_max <- D_m <- D_ve <- D_st <- D_lo <- D_mi <- D_hi <- a_cs <- I_o <- T_opt <- T_min <- T_max <- S_opt <- S_min <- S_max <- h_a <- h_b <- h_c <- h_max <- DWWW <- as.numeric(NA)
```
...
#| fig-cap: |
#|    Uptake of ammonium (green), nitrate (blue) or amino acids (red) by _A. armata_ via Michaelis–Menten kinetics (solid lines) or a linear rate (dashed lines) with increasing substrate concentration (maximum of 30$\mu$M). Dotted lines show reference concentrations of 5, 20 and 50 $\mu$M for reference.
...
This gives a minimum N of `r round(100*N_min,2)`% (a $Q_{min}$ of `r round(Q_min, 2)` mg gDW$^{-1}$) and a maximum N-content of `r round(100*N_max,2)`% (a $Q_{max}$ of `r round(Q_max, 2)` mg gDW$^{-1}$), which is a relatively high value for macroalgae.
...
h <- sapply(X = dens, FUN = algae_height, spec_params = c(h_a = h_a, h_b = h_b, h_c = h_c, h_max = drop_units(h_max)))
...
```{r scale max growth}
#| code-summary: Scale max growth with Q_lim

Qlim <- sapply(X = 1:250, FUN = Q_lim, Nf = 50, spec_params = drop_units(c(Q_min = Q_min, Q_max = Q_max, K_c = Q_min * 0.5))) 
plot(Qlim)
```
...
line_a <- nls(formula = as.formula(mean ~ a * as.numeric(label)),
    start = c(a = 0.1),
    data = hall_gam_vel2)
...
| Parameter | Units | _A. armata_ value | _A. taxiformis_ value | Sources |
|-----------|:-----:|:--------------:|:--------------:|:-------------------------------|
| $V_{am}$  | `r as.character(units(V_am))`  | `r format_sci(drop_units(V_am))` | | @schuenhoff_tetrasporophyte_2006 |
| $K_{am}$  | `r as.character(units(K_am))`  | `r format_sci(drop_units(K_am))` | | @schuenhoff_tetrasporophyte_2006 |
| $M_{am}$  | `r as.character(units(M_am))`  | `r format_sci(drop_units(M_am))` | | @torres_nitrogen_2021 |
| $C_{am}$  | `r as.character(units(C_am))`  | `r format_sci(drop_units(C_am))`   | | @torres_nitrogen_2021 |
| $M_{ni}$  | `r as.character(units(M_ni))`  | `r format_sci(drop_units(M_ni))`   | | @torres_nitrogen_2021 |
| $C_{ni}$  | `r as.character(units(C_ni))`  | `r format_sci(drop_units(C_ni))`   | | @torres_nitrogen_2021 |
| $M_{ot}$  | `r as.character(units(M_ot))`  | `r format_sci(drop_units(M_ot))`   | | @torres_nitrogen_2021 |
| $C_{ot}$  | `r as.character(units(C_ot))`  | `r format_sci(drop_units(C_ot))`   | | @torres_nitrogen_2021 |
| $K_{c}$   | -                              | `r format_sci(K_c)`              | | @hadley_modeling_2015 |
| $Q_{min}$ | `r as.character(units(Q_min))` | `r format_sci(drop_units(Q_min))` | | @mihaila_new_2022; @zemke-white_chlorophyte_1999; @angell_protein_2016 |
| $Q_{max}$ | `r as.character(units(Q_max))` | `r format_sci(drop_units(Q_max))` | | @schuenhoff_tetrasporophyte_2006 |
| $DWWW$    | `r as.character(units(DWWW))`  | `r format_sci(drop_units(DWWW))` |  | @nunes_evaluation_2024; @roque_effect_2019 |
| $\mu$     | `r as.character(units(mu))`    | `r format_sci(drop_units(mu))`               | |  |
| $D_{m}$   | `r as.character(units(D_m))`   | `r format_sci(drop_units(D_m))`    | |  |
| $D_{v}$   | `r as.character(units(D_ve))`  | `r format_sci(drop_units(D_ve))`    | |  |
| $D_{lo}$  | `r as.character(units(D_lo))`  | `r format_sci(drop_units(D_lo))` | |  |
| $D_{mi}$  | `r as.character(units(D_mi))`  | `r format_sci(drop_units(D_mi))` | |  |
| $D_{hi}$  | `r as.character(units(D_hi))`  | `r format_sci(drop_units(D_hi))` | |  |
| $I_o$     | `r as.character(units(I_o))`   | `r format_sci(drop_units(I_o))`    | |  |
| $a_{cs}$  | `r as.character(units(a_cs))`  | `r format_sci(drop_units(a_cs))`    | |  |
| $T_{opt}$ | `r as.character(units(T_opt_arma))` | `r format_sci(drop_units(T_opt_arma))` | `r format_sci(drop_units(T_opt_taxi))` |  |
| $T_{min}$ | `r as.character(units(T_min_arma))` | `r format_sci(drop_units(T_min_arma))` | `r format_sci(drop_units(T_min_taxi))` |  |
| $T_{max}$ | `r as.character(units(T_max_arma))` | `r format_sci(drop_units(T_max_arma))` | `r format_sci(drop_units(T_max_taxi))` |  |
| $S_{opt}$ | `r as.character(units(S_opt))` | `r format_sci(drop_units(S_opt))` | |  |
| $S_{min}$ | `r as.character(units(S_min))` | `r format_sci(drop_units(S_min))` | |  |
| $S_{max}$ | `r as.character(units(S_max))` | `r format_sci(drop_units(S_max))` | |  |
| $h_a$     | -                              | `r format_sci(h_a)` | |  |
| $h_b$     | -                              | `r format_sci(h_b)` | |  |
| $h_c$     | -                              | `r format_sci(h_c)` | |  |
| $h_{max}$ | `r as.character(units(h_max))` | `r format_sci(drop_units(h_max))` | |  |
: Final values for species parameters before saving. These are displayed both for transparency and to check that the units have converted correctly. {#tbl-params}

## Saving data {#sec-save}
...
* Dissolved inorganic carbon may be a major (perhaps determining?) limiting factor for growth and photosynthesis in _Asparagopsis_ [@zanolla_concise_2022; @mata_direct_2010]
...
This document was written in Quarto [@quarto]. All code used R version `r R.version[['major']]`.`r R.version[['minor']]` "`r R.version[['nickname']]`" [@R_base], and major packages used include `r cite_packages(packs)`.
